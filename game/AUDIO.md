...
BGM とほとんどの効果音は Web Audio による手続き生成。ただし**打撃音と足音
だけは録音素材**を使っている (`audio/`)。合成で3度作り直しても「ポーン」と
柔らかい音にしかならず、質的に届かなかったため。

録音素材はそのまま鳴らすのではなく、**合成のサブベースを重ねている**。
録音は質感と立ち上がりを、合成は低域の重さを担当する。素材が読み込めて
いない間は合成側だけで鳴るので無音にはならない (`sampledHit` の fallback)。

素材はすべて CC0 (Kenney / Still North Media)。出典は `audio/CREDITS.md`。

### 実録素材を足すときの注意

- **`fade` を指定して尾を刈ること。** 実録は余韻が長く、連打すると滲む。
  刃鳴りの素材は素で 1.4 秒鳴っていた
- 再生ごとに `rate` を少し散らすと同じ音の連発に聞こえない
- 低域は素材に期待せず `sub` で足す
...
| `field4` | 4章 フィールド | 黄昏の塔。低い唸りと遠い鐘 |
| `field5` | 5章 フィールド | 深淵の洞。地鳴りと、点で落ちる水滴 |
| `battle1..5` | 通常戦闘 (章別) | 162BPM / 16小節。章ごとに和音・旋律・音色を丸ごと差し替え |
| `boss1..5` | ボス戦 (章別) | 172BPM。旋律は共通で、章ごとに移調(±0 / −3 / +4 / −5 / −8)と音色を変更 |
| `clear` | 章クリア画面 | ハ長調の明るいループ |
| `over` | ゲームオーバー | 沈む挽歌 |

`state.mode` が `'battle'` で `enemies` に `boss:true` が含まれれば boss、
それ以外の戦闘は battle。章は `state.chapter` (1..5 にクランプ)。

### 通常戦闘曲の構造

16小節を **2小節 × 8スロット**に分け、`BATTLE_SECTIONS` が各スロットの
セクション (`A` / `B` / `C`) を決める。`C` は最後の2小節で、旋律を1オクターブ
上げてフィルを入れる。章ごとの素材は `BATTLE_SETS[1..5]` にまとまっている
