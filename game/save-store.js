...
export function writeSlot(slot, data, storage = defaultStorage()) {
  ...
  const payload = JSON.stringify({ ...data, version: SAVE_VERSION, slot: Number(slot), savedAt: Number(data?.savedAt) || Date.now() });
  try {
...
export function slotSummary(slot, storage = defaultStorage()) {
  ...
  return {
    ...
    hero: { lv: Number(data.hero?.lv) || 1, hp: Number(data.hero?.hp) || 0, maxHp: Number(data.hero?.maxHp) || 0 },
    companion: data.companion?.active || data.chapter >= 2
      ? { lv: Number(data.companion?.lv) || 1, hp: Number(data.companion?.hp) || 0, maxHp: Number(data.companion?.maxHp) || 0 }
      : null,
  };
}
