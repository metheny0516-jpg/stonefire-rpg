...
export function selectEnemyAction(enemy, random = Math.random) {
  ...
  if (enemy.behavior === 'charge') {
    if (enemy.charging)
      return { type: 'single', label: '影牙の飛びかかり', multiplier: 1.95, clearsCharge: true };
  ...
  if (enemy.behavior === 'astralBoss') {
    ...
    if (hpRate <= 0.45 && random() < 0.32)
      return { type: 'single', label: '六翼の断空', multiplier: 0.78, hits: 3 };
  }
...
export function rollEncounterGroup(pool, chapter = 1, random = Math.random) {
  ...
  const count = chapter >= 2 ? (sizeRoll < 0.15 ? 3 : sizeRoll < 0.65 ? 2 : 1) : sizeRoll < 0.45 ? 2 : 1;
